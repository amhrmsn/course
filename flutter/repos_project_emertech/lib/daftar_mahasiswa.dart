...
class _DaftarMahasiswaState extends State<DaftarMahasiswa> {
  ...
  Future<String> fetchData() async {
    final response = await http.get(Uri.parse("https://ubaya.cloud/flutter/160421077/UAS/fetch_account.php"));
  ...
  List<Widget> listProfilesCard(BuildContext context) {
    List<Widget> list = [];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      body: SingleChildScrollView(
        child: Center(
          child: Column(
            children: listProfilesCard(context),
          ),
        ),
      ),
    );
